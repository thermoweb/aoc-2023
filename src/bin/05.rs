...
impl Convertor {
    ...
    fn get_convertor(map: &str) -> Convertor {
        ...
        Convertor { input_range: source_start..range_end, output_range: target_start }
    }
}
...
impl ConvertorMap {
    fn get(&self, source: u64) -> Option<u64> {
        let convertor = self.convertors.iter().find(|c| c.input_range.contains(&source));
        if convertor.is_some() {
            return convertor.unwrap().get(source);
        }
        Some(source)
    }
    ...
    fn translate(&self, ranges: Vec<RangeMap>) -> Vec<RangeMap> {
        ...
        ranges.iter().for_each(|input_range| {
            ...
            for convertor in &self.convertors {
                if convertor.input_range.start < input_range.end && convertor.input_range.end > input_range.start {
...
pub fn part_one(input: &str) -> Option<u32> {
    ...
    init
        ...
        .map(|seed| convertors_map.get("seed-to-soil").unwrap().get(seed).unwrap())
        .map(|seed| convertors_map.get("soil-to-fertilizer").unwrap().get(seed).unwrap())
        .map(|seed| convertors_map.get("fertilizer-to-water").unwrap().get(seed).unwrap())
        .map(|seed| convertors_map.get("water-to-light").unwrap().get(seed).unwrap())
        .map(|seed| convertors_map.get("light-to-temperature").unwrap().get(seed).unwrap())
        .map(|seed| convertors_map.get("temperature-to-humidity").unwrap().get(seed).unwrap())
        .map(|seed| convertors_map.get("humidity-to-location").unwrap().get(seed).unwrap())
        .map(|r| r as u32)
        .min()
}
...
pub fn part_two(input: &str) -> Option<u32> {
    ...
    let fert = convertors_map.get("soil-to-fertilizer").unwrap().translate(soil);
    let water = convertors_map.get("fertilizer-to-water").unwrap().translate(fert);
    let light = convertors_map.get("water-to-light").unwrap().translate(water);
    let temp = convertors_map.get("light-to-temperature").unwrap().translate(light);
    let hum = convertors_map.get("temperature-to-humidity").unwrap().translate(temp);
    let location = convertors_map.get("humidity-to-location").unwrap().translate(hum);
...
mod tests {
    ...
    #[test]
    fn test_convertor() {
        let convertor = Convertor { output_range: 10, input_range: 1..9 };
